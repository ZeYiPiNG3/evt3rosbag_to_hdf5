...
class Event:
    def __init__(self, h5file, topic, bag):
        ...
        self.trig_event_ds = self.group.create_dataset(
            "trig_event",
            (event_count,),  
            compression="lzf",
            dtype="i8",  
    ...
    def process(self, msg, topic=None, bag_time=None):
        ...
        if trig_events.shape[0] > 0:
            self.trig_event_buf.append(trig_events['t'])  # 假设我们只关心时间戳
    ...
    def finish(self):
        ...
        trig_event = np.concatenate(self.trig_event_buf)
        self.trig_event_ds[:trig_event.shape[0]] = trig_event
        self.trig_event_ds.resize(trig_event.shape)
